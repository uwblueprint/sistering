...
const VolunteerPostingsPage = (): React.ReactElement => {
  ...
  useEffect(() => {
    let filteredPostings;
      switch (filter) {
        case 'month': 
          filteredPostings = postings?.filter(posting => posting.startDate.getTime() - Date.now() > 0 && posting.startDate.getTime() - Date.now() < MS_PER_WEEK * 4)
        ...
        case 'all':
          ...
          break;
        default:
          filteredPostings = postings?.filter(posting => posting.startDate.getTime() - Date.now() > 0 && posting.startDate.getTime() - Date.now() < MS_PER_WEEK)
          setPostings(filteredPostings ?? null)
          break;
      }
