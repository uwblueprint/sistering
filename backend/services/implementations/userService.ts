    ...
    try {
      try {
        ...
        if (
          ...
          errMsg = `Error occurred that was not from firebase regarding not finding email: ${getErrorMessage(
            error,
          )}`;
        }
      }
      ...
      throw new Error(`Failed to create user invite row. Reason = ${errMsg}`);
    }
  }
